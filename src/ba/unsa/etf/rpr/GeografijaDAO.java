...
public class GeografijaDAO {
    ...
    private PreparedStatement glavniGradUpit, dajDrzavuUpit, obrisiDrzavuUpit, obrisiGradoveZaDrzavuUpit, nadjiDrzavuUpit,
            ...
            promijeniDrzavuUpit, dajGradUpit, nadjiGradUpit, obrisiGradUpit, dajDrzaveUpit;
    ...
    private GeografijaDAO() {
        ...
        try {
            glavniGradUpit = conn.prepareStatement("SELECT grad.id, grad.naziv, grad.broj_stanovnika, grad.drzava, grad.luka FROM grad, drzava WHERE grad.drzava=drzava.id AND drzava.naziv=?");
        } catch (SQLException e) {
            regenerisiBazu();
            try {
                glavniGradUpit = conn.prepareStatement("SELECT grad.id, grad.naziv, grad.broj_stanovnika, grad.drzava, grad.luka FROM grad, drzava WHERE grad.drzava=drzava.id AND drzava.naziv=?");
        ...
        try {
            ...
            promijeniGradUpit = conn.prepareStatement("UPDATE grad SET naziv=?, broj_stanovnika=?, drzava=?, luka=? WHERE id=?");
            promijeniDrzavuUpit = conn.prepareStatement("UPDATE drzava SET naziv=?, glavni_grad=?, pomorska=? WHERE id=?");
        } catch (SQLException e) {
            e.printStackTrace();
        }
